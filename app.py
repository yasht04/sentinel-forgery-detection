...
def dct_frequency_analysis(image_pil):
    ...
    dct_coeffs = scipy_dct(
        scipy_dct(img_gray, axis=0, norm="ortho"), axis=1, norm="ortho"
    )
    ...
    smoothness = 1.0 / (
        np.std(np.array(image_pil.convert("RGB")).astype(np.float32)) / 50.0 + 1.0
    )
...
def run_sbi_inference(image_pil, caption, model, tokenizer, device):
    ...
    with torch.no_grad():
        logit, heatmap = model(
            img_tensor, enc["input_ids"].to(device), enc["attention_mask"].to(device)
        )
    return torch.sigmoid(logit).item(), heatmap.squeeze().cpu().numpy(), image_np


def analyze_caption_image_consistency(
    ...
    try:
        ...
        inp1 = clip_processor(
            text=alternatives, images=image_pil, return_tensors="pt", padding=True
        )
        with torch.no_grad():
        ...
        inp2 = clip_processor(
            text=hard_negatives, images=image_pil, return_tensors="pt", padding=True
        )
        with torch.no_grad():
        ...
        for word in key_words:
            ...
            inp_w = clip_processor(
                text=all_templates, images=image_pil, return_tensors="pt", padding=True
            )
            with torch.no_grad():
                out_w = clip_model(**inp_w)
                pr_w = torch.softmax(out_w.logits_per_image.squeeze(0), dim=0)

            pos_score = float(pr_w[:4].sum().item())
            neg_score = float(pr_w[4:].sum().item())
...
def combine_scores(sbi, clip_s, dct, clip_ok, caption_analysis=None):
    ...
    if caption_analysis is not None:
        ...
        mismatch_signal = max(0.0, 0.5 - match)
...
def get_verdict(sbi, clip_s, dct, final, threshold, clip_ok, caption_analysis=None):
    ...
    if dominant == "CLIP" and clip_s > 0.55:
        return "AI_GENERATED"
    if dominant == "CAPTION" and clip_ok:
        return "AI_GENERATED"
    if dominant == "SBI" and sbi > 0.55:
        return "SPLICED"
    return "MANIPULATED"
...
def layer_color_class(s):
    if s > 0.65:
        return "layer-row-red", "#ff3b3b", "HIGH RISK"
    if s > 0.45:
        return "layer-row-amber", "#f0a500", "ELEVATED"
...
with st.sidebar:
    st.markdown(
        '<div class="sidebar-title">⚙ SYSTEM STATUS</div>', unsafe_allow_html=True
    )

    layers = [
        ("L1 SBI SPLICE DETECTOR", "ONLINE", "#00e676"),
        (
            "L2 CLIP SEMANTIC ENGINE",
            "ONLINE" if clip_ok else "OFFLINE",
    ...
    st.markdown(
        '<div class="sidebar-title">📊 MODEL STATS</div>', unsafe_allow_html=True
    )
    for label, val in [
        ("ACCURACY", "93.1%"),
        ("F1 SCORE", "0.9285"),
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        '<div class="sidebar-title">🎚 SENSITIVITY</div>', unsafe_allow_html=True
    )
    threshold = st.slider(
        "Detection threshold", 0.10, 0.90, 0.45, 0.05, label_visibility="collapsed"
    )
    st.markdown(
        f"""
...
with col_left:
    ...
    if uploaded_file is not None:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            '<div class="sec-label">▸ INPUT PREVIEW</div>', unsafe_allow_html=True
        )
...
with col_right:
    st.markdown(
        '<div class="sec-label">▸ ANALYSIS REPORT</div>', unsafe_allow_html=True
    )
    ...
    if uploaded_file is not None and analyze_btn:
        ...
        with st.spinner("[ L2 ] RUNNING CLIP SEMANTIC ANALYSIS..."):
            clip_score, _ = (
                clip_ai_detection(image_pil, clip_model, clip_processor)
                if clip_ok
                else (0.5, False)
            )
        ...
        final_score = combine_scores(
            sbi_score, clip_score, dct_score, clip_ok, caption_analysis
        )
        verdict = get_verdict(
            sbi_score,
            clip_score,
        ...
        st.markdown(
            '<div class="sec-label">▸ LAYER ANALYSIS</div>', unsafe_allow_html=True
        )

        layers_data = [
            (
                "🔬",
                "L1 — SBI SPLICE DETECTOR",
                "Copy-paste & region splicing detection",
                sbi_score,
                True,
            ),
            (
                "🧠",
                "L2 — CLIP SEMANTIC ENGINE",
                "GAN · Diffusion · MidJourney · DALL-E",
                clip_score,
                clip_ok,
            ),
            (
                "📡",
                "L3 — DCT FREQUENCY ANALYZER",
                "Unnatural frequency pattern detection",
                dct_score,
                True,
            ),
        ]
        ...
        st.markdown(
            '<div class="sec-label">▸ SPLICE LOCALIZATION MAP</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            '<div class="sec-label">▸ FORENSIC SUMMARY</div>', unsafe_allow_html=True
        )
...
if uploaded_file is not None and analyze_btn:
    ...
    st.markdown(
        '<div class="sec-label">▸ CAPTION — IMAGE CONSISTENCY ANALYSIS</div>',
        unsafe_allow_html=True,
    )
    ...
    if not caption.strip():
        ...
        if match_score > 0.65:
            ...
            cons_desc = (
                "Caption partially matches — some elements may be misrepresented"
            )
            cons_icon = "◐"
        else:
            cons_color = "#ff3b3b"
        ...
        with cb:
            ...
            if not mismatch_words and matched_words:
                interp_parts.append(
                    "Caption appears to accurately describe this image."
                )
    ...
    st.markdown("---")
    st.markdown(
        '<div class="sec-label">▸ MODEL PERFORMANCE — PRESENTATION OVERVIEW</div>',
        unsafe_allow_html=True,
    )
